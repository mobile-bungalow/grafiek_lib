pub fn draw_grid(viewport: &egui::Rect, style: &egui::Style, painter: &egui::Painter) {
    ...
    let start_x = (viewport.left() / spacing).floor() * spacing;
    let start_y = (viewport.top() / spacing).floor() * spacing;

    let x_iter = 0..(viewport.right() / spacing).trunc() as usize;
    let y_iter = 0..(viewport.bottom() / spacing).trunc() as usize;
